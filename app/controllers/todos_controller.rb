class TodosController < ApplicationController
  ...
  def index
    ...
    if cookies[:udemy_time_check32].blank? and request.os == 'Android' and @userid.to_s == "1"
      begin
        scrap = Scrap_check.new
        url = "https://www.udemy.com/ja"
        udemy_check = scrap.check(url,"対象コース","セール","セール")
        cookies[:udemy_time_check32] = { :value => udemy_check, :expires => 1.days.from_now }
        if cookies[:udemy_time_check32] 
